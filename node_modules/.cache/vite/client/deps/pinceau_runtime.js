...
import { ref, computed, watch, onScopeDispose, unref as unref2, getCurrentInstance, inject } from "vue";
    ...
    for (const [varName, _value] of Object.entries(computedStyles)) {
      ...
      if (typeof value === "object") {
        for (const [mqId, mqPropValue] of Object.entries(value)) {
          const _value2 = unref2(mqPropValue);
          if (!_value2) {
            continue;
          }
          if (mqId === "initial") {
...
var plugin = {
  install(app, options) {
    ...
    if (dev && (import.meta.hot || process.server)) {
      import("./debug-XZTFPOQY.js").then(({ usePinceauRuntimeDebug }) => usePinceauRuntimeDebug(tokensHelperConfig));
    }
